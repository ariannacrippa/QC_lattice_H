...
class HamiltonianQED(HCLattice):
    ...
    def __init__(
        ...
        self.q_charge_str_list = ["q_" +"".join(map(str, node))  for node in self.graph.nodes if self.puregauge is False]
        self.static_charges_str_list = [
                "Q_" +"".join(map(str, node))  for node in self.graph.nodes
                if  self.static_charges_values is not None
            ]
        # Dictionary of all elements (E and charges q) in the system with their symbols
        self.e_op_dict = {
            s_tmp: symbols(s_tmp)
            for s_tmp in self.list_edges2_e_op
        ...
        self.u_op_dict = {
            s_tmp: symbols(s_tmp)
            for s_tmp in self.list_edges2_u_op
        ...
        self.e_op_free = list(
            set([symbols(j) for j in self.list_edges2_e_op]).intersection(
                set(
                    [
                        item
                        for sublist in [
        ...
        self.u_op_free = [
            k.subs(
                [
                    (symbols(j), symbols(k))
                    for j, k in zip(self.list_edges2_e_op, self.list_edges2_u_op)
                ]
            )
...
#TODO FROM HERE
    ...
    # Gauss law equations in a list # TODO: test for non-zero total charge
    def gauss_equations(self):
        ...
        for node in self.graph.nodes:
            if self.puregauge:
                ...
                ga_tmp=-1*self.e_op_dict['q_'+"".join(map(str, node))]
                gc_tmp += self.e_op_dict['q_'+"".join(map(str, node))]
            if self.static_charges_values is not None:
                ga_tmp-=1*self.e_op_dict['Q_'+"".join(map(str, node))]
                gc_tmp+=self.e_op_dict['Q_'+"".join(map(str, node))]

            e_op_i = "E_" +"".join(map(str, node))
            for j, k in zip(self.list_edges, self.list_edges2_e_op):
                        if e_op_i in j:
                            if e_op_i == j[0]:  # E_out
                                coeff = (
