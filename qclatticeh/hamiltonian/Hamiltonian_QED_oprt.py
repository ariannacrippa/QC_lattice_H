...
class HamiltonianQED_oprt:
    ...
    def __init__(
        ...
        self.new_encoding = (config["new_encoding"] if "new_encoding" in config else False)
        ...
        ext_enc_set = [2**n - 1 for n in range(1, 10)]+[0,]
    ...
    def _n_qubits_g(self) -> int:
        ...
        return int(np.ceil(np.log2(2 * self.l_par + 1))) if self.l_par>0 else 1 #TODO: l=0 for Z_2
    ...
    def l_c2(self):
        """Ladder function equivalent to
        Defined for Gray encoding test new encoding 0_ph=01+10/sqrt2"""
        states_list = list(
            range(-self.l_par, self.l_par + 1 + self.ext_fact)
        )  # states: -l, ..., l
        u_op_list = []
        for st_fact in states_list:
            if st_fact != -self.l_par:
                # print('stfact',gray_map()[st_fact])
                encs = [self._gray_map()[st_fact - 1], self._gray_map()[st_fact]]

                are_tuples = list(map(lambda x: isinstance(x, tuple), encs))
                nterms = 1 << sum(are_tuples)
                # print(nterms)
                fact = 1.0 / np.sqrt(2)#TODO for l generic
                for ei_tmp, enc in enumerate(encs):
                    if not are_tuples[ei_tmp]:
                        encs[ei_tmp] = (enc,)
                for e1_tmp in encs[0]:
                    for e2_tmp in encs[1]:
                        # print(fact, e1_tmp, e2_tmp)
                        u_op_list.append(
                            [fact, *HamiltonianQED_oprt._trans_map(e1_tmp, e2_tmp)]
                        )

        for encs in [['00','10'],['10','11']]:
            are_tuples = list(map(lambda x: isinstance(x, tuple), encs))
            nterms = 1 << sum(are_tuples)
            # print(nterms)
            fact = 1.0 / np.sqrt(2)#TODO for l generic
    ...
    def u_op_dag_enc(self):
        ...
        if self.encoding == "gray":
            if self.new_encoding:
                self.u_oper_dag = self.str_to_pauli(self.l_c2(), self._n_qubits_g())
            else:
    ...
    def u_op_enc(self):
        ...
        if self.encoding == "gray":
            if self.new_encoding:
                self.u_oper = self.str_to_pauli(self.l_c2(), self._n_qubits_g()).adjoint()
            else:
